...
compose (Lens get1 set1) (Lens get2 set2) =
  Lens (get2 . get1) (\c a -> set1 (set2 c $ get1 a) a)
...
data Student = Student {name :: String, address :: Address} deriving (Show)

newtype Professor = Professor {name :: String} deriving (Show)
...
studentToAddress = Lens address (\a s -> Student (name (s :: Student)) a)
...
setVL lens b a = runIdentity $ lens (const $ Identity b) a
